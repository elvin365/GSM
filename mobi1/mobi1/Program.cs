...
namespace mobi1
{
    class Program
    {
        static public byte[] A5Encyptor(byte[] msg, byte[] key)
        {
            ...
            int framesCount = msg.Length*8 / 228 +1;
        ...
        class A5Enc
        {
            ...
            public void KeySetup(byte[] key, int[] frame)
            {
                ...
                for (int i = 0; i < 64; i++)
                {
                    clockall();// сдвигаем регистры на 1 позицию игноря биты синхронизации
                ...
                for (int i = 0; i < 100; i++)
                {
                    clock();//сдвиг регистров на 1 позиц с учетом битов синхронизации
                }
            }
        ...
        {
            // generate 128 bit ki
            byte[] ki = Encoding.ASCII.GetBytes("NOTMySecretPassword");//"MySecretPassword"
           ...
           // r.NextBytes(rand);
            ...
            try
            {
                ...
                rand = data;
                Console.WriteLine("Received rand:" + BitConverter.ToString(rand).Substring(0,47));
                // make sres
                Comp128(rand, ki, ref sres, ref kc);
                Console.WriteLine("Make sres: {0}",sres);

                // send sres
                socket.Send(BitConverter.GetBytes(sres));

                // receive result
                bytes = socket.Receive(data, data.Length, 0);
                if (Encoding.ASCII.GetString(data).Substring(0,2) == "ok")
                {
                    Console.WriteLine("Server accepted client's sres");
                    Console.WriteLine("Set session key: {0}", kc );
                    string input = "close";
                    do
                    {
                        Console.Write(">");
                        input = Console.ReadLine();
                        var emsg = A5Encyptor(Encoding.ASCII.GetBytes(input), BitConverter.GetBytes(kc));
                        socket.Send(emsg);
                        bytes = socket.Receive(data);
                        byte[] tmp = new byte[bytes];
                        Array.Copy(data, 0, tmp, 0, bytes);
                        var dmsg = A5Encyptor(tmp, BitConverter.GetBytes(kc));
                        string res = Encoding.ASCII.GetString(dmsg);
